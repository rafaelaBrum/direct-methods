...
void acharFormaCanonica(double*);
...
void acharFormaCanonica(double *pMat)
{
    int l, i, j, k, index;

    acharFormaEscalonada(pMat);
    //imprimirMatriz(pMat);
    ...
    for(l = tam-1; l >= 0; l--)
    {
        for(i = l-1; i >=0; i--)
        {
            *(pMat + (tamC*i) + tam) -= (*(pMat + (tamC*i) + l) * (*(pMat + (tamC*l) + tam)) / (*(pMat + (tamC*l) + l)));
            for(j = l; j >= 0; j--)
            {
                *(pMat + (tamC*i) + j) -= (*(pMat + (tamC*i) + l) * (*(pMat + (tamC*l) + j)) / (*(pMat + (tamC*l) + l)));
            }
        }

        for(i = tam; i >= l; i--)
        {
            *(pMat + (tamC*l) + i) /= (*(pMat + (tamC*l) + l));
        }
    }
}

void imprimirMatriz(double *mat)
{
    int i, j;
    for(i = 0; i < tam; i++)
    {
